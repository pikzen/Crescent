...
bool CColorShader::Render(ID3D11DeviceContext* deviceContext, int indexCount, D3DXMATRIX wMatrix, D3DXMATRIX vMatrix, D3DXMATRIX pMatrix)
{
	bool result;
...
bool CColorShader::InitializeShader(ID3D11Device* device, HWND hwnd, char* vsFilename, char* psFilename)
{
	HRESULT result;
	...
	if (FAILED(result))
	{
		if (errorMessage) 
			...
			CLog::Write("CColorShader::InitializeShader() : Missing vertex shader file");
		return false;
	}
	...
	if (FAILED(result))
	{
		if (errorMessage)
			...
			CLog::Write("CColorShader::InitializeShader() : Missing pixel shader file");
		return false;
	}
...
void CColorShader::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, char* shaderFilename)
{
	char* compileErrors;
...
bool CColorShader::SetShaderParameters(ID3D11DeviceContext* deviceContext, D3DXMATRIX wMatrix, D3DXMATRIX vMatrix, D3DXMATRIX pMatrix)
{
	HRESULT result;
